...
const helpContent = {
	shellType: {
		header: '烟花类型',
		body: '要燃放的烟花类型，默认为“随机(Random)”。'
	},
	shellSize: {
		header: '烟花大小',
		body: '烟花越大绽放范围就越大，设备所需的性能也会增多，大的烟花可能会导致你的设备卡顿。'
	},
	quality: {
		header: '动画画质',
	...
	finaleMode: {
		header: '更多烟花',
		body: '可以在同一时间自动放出更多的烟花，但需要开启先开启“自动燃放”。'
	},
	hideControls: {
		header: '隐藏按钮',
...
const str = document.querySelector('.wishContent').innerHTML
const showStr = document.querySelector('.showContent')
...
const textPlay = () => {
	time = setInterval(move, 120)
	play = setInterval(cursor, 180)
}

const move = () => {
	showStr.innerHTML = str.substring(0, index++)
...
const cursor = () => {
	textSpan.innerHTML = '|'
	showStr.appendChild(textSpan)
	textSpan.classList.toggle('.sign')
}
...
const pauseBtnCover = document.querySelector('.pause-btn-cover');
const pauseBtnUse = document.querySelector('.pausebtn');
const soundBtnCover = document.querySelector('.sound-btn-cover');
const soundBtnUse = document.querySelector('.soundbtn');
const settingsBtnCover = document.querySelector('.settings-btn-cover');
const settingsBtnUse = document.querySelector('.settingsbtn');
const wishTextShow = document.querySelector('.wishText');

// 播放
pauseBtnCover.addEventListener('click', function() {
	if(pauseBtnUse.getAttribute('href') === '#icon-pause') {
		textPlay()
		musicPlay.play()
	} else if(pauseBtnUse.getAttribute('href') === '#icon-play') {
		clearInterval(time)
		clearInterval(play)
		musicPlay.pause()
	}
...
// 音量
soundBtnCover.addEventListener('click', function() {
	if(soundBtnUse.getAttribute('href') === '#icon-sound-off') {
		musicPlay.muted = true;
	} else if (soundBtnUse.getAttribute('href') === '#icon-sound-on') {
		musicPlay.muted = false;
	}
})

// 设置
settingsBtnCover.addEventListener('click', function() {
	wishTextShow.classList.toggle('hide');
})
